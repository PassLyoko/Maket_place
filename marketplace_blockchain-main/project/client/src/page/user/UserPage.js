...
export default function UserPage(props){
    ...
    useEffect(() => {
      ...
      const handCLickDelivered = async (item) =>{
        try{
          ...
          const res1 = await axios.patch(`http://localhost:5000/api/products/buyer/update/${item.addressItem}`).then(res =>{ 
            console.log("buyer update Success")
          });

          props.handleChangeLogin(false);
    
          //toast
          toast.success("Delivered Item Success!!!", {
      ...
      const handCLickDelete = async (item) =>{
        try{
          ...
          await axios.delete(`http://localhost:5000/api/products/productdelete/${item.addressItem}`)
          ...
          setTimeout(function() {
             window.location.reload(false)}, 3000);
    ...
    <div className="tables_dashboard">
      <div className="table_products">
        <ListProducts listItems={listItems} handCLickDelivered = {handCLickDelivered} handCLickDelete = {handCLickDelete}></ListProducts>
      </div>
      <div className="table_products">
